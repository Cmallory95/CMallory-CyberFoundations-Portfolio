...
```
When I logged in a grey and black daunting screen greeted me lol. The screen seemed to give information about the VM session, for instance it said Welcome to Ubuntu 22.04.5 LTS 
```
...
```
the hostname showed that it was for me because I am student-08 it also proved that I am the analyst of the room by runnning whoami lastly it showed me I am in my own room with pwd it showed I am in the home drive of my analyst network
```
...
```
A golden standard image can be looked at as a template. It allows for multiple workstations to have the same set up for uniformity. This is key for an org because if they are using multiple different VMs it would be helpful to ensure they all have the same set up instead of recreating it is faster as well. 
```
...
```
What will stay the same is the original set up of it being a Ubuntu VM what will change is the data and information we have, and other things pertinent to us only such as VM IP addresses etc
```
...
```
Restoring comes in handy when you are about to make changes or are unsure if the changes you make will stick. Restoring allows you to bring the VM back to a known good place so you can re-attempt whatever you are trying to do. It is safe because you are using a template in essence and not trying to remember how things were before the change to try and manually restore your previous version
```
...
```
A snapshot is held in the same location as all of your files, a backup is a seperate location. if in the event of a crash or deletion, your backup will not be lost but your screenshot will be. Snapshot helps you to edit undo. Backup helps you to fully restore if in the event the system fails/ goes down.
```
...
```
Reserved for you means you can rent it and multiple people can use it. When someone creates something from scratch they usually do not want others in it or on it meaning you need more space/ power/ etc

With it being reserved, when said person is done all their info is deleted and the room is reset and ready for the next person like a hotel room. This allows for easier flexibility and scalability for organizations.

Relate: A hotel makes more than someone's Airbnb home.
```
