...
**My Diagram:** hardware-os-software-diagram.png
...
```
My diagram is a simple expression of how it all starts with the CPU (central processing unit) which boots up the Operating system. The CPU is full of hardware, which communicates with software that is installed in the CPU (in the memory/ RAM). The software tells the hardware what to do/ how to operate. 
```
...
```
If the OS crashed the hardware and software would not launch. The CPU may still turn on on but with no interface/ OS you would not be able to do anything. 
```
...
```
I use teams almost everyday for work. If the hardware layer failed the application would not load. It may also give errors when trying to load/ boot or have other issues like force closing etc. 
```
...
```
Understanding all three layers can help you to be proactive instead of reactive you can check the security frameworks and updates of the applications to help mitigate issues. Also everyone understands things on different levels a developer maybe comfortable on the hardware/ software level but an everyday user may only understand the OS/interface. 
```
...
```
It is the layout of how people, devices, networks etc all work together and the importance of keeping our data safe from threat actors.
```
...
```
Welcome center-- it all starts with hardware/OS/software
```
